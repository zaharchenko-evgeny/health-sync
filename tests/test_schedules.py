...
def test_schedule_settings_default_to_daily_zepp_and_three_hour_yazio(monkeypatch):
    ...
    assert settings.yazio_interval_minutes == 180


def test_yazio_interval_falls_back_to_legacy_serve_interval(monkeypatch):
    monkeypatch.delenv("HEALTH_SYNC_YAZIO_INTERVAL_MINUTES", raising=False)
    monkeypatch.setenv("HEALTH_SYNC_SERVE_INTERVAL_MINUTES", "240")

    settings = Settings.from_env()

    assert settings.yazio_interval_minutes == 240
...
def test_scheduled_deployments_use_separate_zepp_and_yazio_schedules(monkeypatch):
    ...
    assert isinstance(yazio_schedule, IntervalSchedule)
    assert yazio_schedule.interval == timedelta(minutes=180)
